...
class HuaweiAICCClient:
    ...
    def _normalize_querycalls_row(cls, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reason_text = str(
            normalized.get("callReason")
            or normalized.get("talkReason")
            or normalized.get("talkRemark")
            or normalized.get("motivo")
            or ""
        ).strip()
        if reason_text:
            normalized.setdefault("callReason", reason_text)
    ...
    @retry(
        ...
        stop=stop_after_attempt(2) | stop_after_delay(60),
        retry=retry_if_exception_type(WafRateLimitError),
        reraise=True
    )
    async def _post_json(
        ...
        Implementa Retry com Backoff Exponencial para lidar com WAF/Rate Limits.
        Usa o singleton HuaweiHttpSession para reuso de conexoes.
        """
        ...
        # O redirecionamento de IP agora é feito via core.network_utils.apply_dns_overrides()
        # para preservar o SNI e evitar erro 418 (CloudWAF).
        
        try:
            client = await HuaweiHttpSession.get_instance()
            resp = await client.post(url, content=body, headers=headers, timeout=to)
            
            # Se atingiu o WAF ou limite de taxa, lancamos excecao para o Tenacity disparar o retry
            if resp.status_code in (403, 429, 502):
                HuaweiEvents.log_event(
                    "DOWNLOAD_FAILED", 
                    call_id=payload.get("callId") or payload.get("msgBody", {}).get("callId"),
                    context={"status_code": resp.status_code, "reason": "WAF_RATE_LIMIT", "url": url},
                    severity="WARNING"
                )
                raise WafRateLimitError(f"Huawei Gateway Error {resp.status_code}")
                
            return resp
            
        except (httpx.HTTPError, WafRateLimitError) as exc:
            if isinstance(exc, WafRateLimitError):
                raise # Deixa o Tenacity lidar
    ...
    async def buscar_historico_chamadas(
        ...
        *,
        ...
        payload["isCallIn"] = "true" if str(call_direction).upper() == "INBOUND" else "false"
    ...
    async def consultar_direcao_chamada(self, call_id: str) -> Optional[bool]:
        """Consulta a direcao REAL da chamada na Huawei por callId.

        Tenta querydetailcallinfo e, em fallback, querybasiccallinfo, varrendo a
        resposta por `isCallIn` -> True (inbound/receptiva), False (outbound/
        ativa) ou None (indisponivel/ambiguo).

        PENDENTE DE VALIDACAO: o formato de resposta dessas APIs ainda nao foi
        confirmado contra a Huawei real (auth so funciona em producao). Rode
        backend/scripts/diag_huawei_direcao_callid.py em producao para validar.
        Helper ISOLADO — ainda NAO integrado ao gate de download (Camada 1)."""
        from core.huawei_direction import extract_is_call_in_from_response

        for endpoint in (
            "/rest/cmsapp/v1/openapi/calldata/querydetailcallinfo",
            "/rest/cmsapp/v1/openapi/calldata/querybasiccallinfo",
        ):
            ...
            try:
                ...
                # Rate limit / rede / nao-JSON: tenta o proximo endpoint e, se
                # ambos falharem, devolve None (chamador cai nas defesas seguintes).
                logger.warning(
                    "consultar_direcao_chamada: falha em %s para callId %s", endpoint, call_id, exc_info=True
                )
                continue
            direction = extract_is_call_in_from_response(data)
    ...
    async def baixar_gravacao_por_callid(self, call_id: str) -> Optional[bytes]:
        ...
        if sentry_sdk:
            sentry_sdk.add_breadcrumb(
                category="huawei_integration",
                message=f"Baixando gravação por call_id via FS",
